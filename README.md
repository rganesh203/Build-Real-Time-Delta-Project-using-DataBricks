...
Streaming tables: 
    Streaming tables process each record exactly once, assuming an append-only source.
Materialized views(MV): 
    Materialized views process records as needed to return accurate results for the current data state and are used for data sources with updates, deletions, or aggregations, as well as for change data capture (CDC) processing.
Views: 
    Views are intermediate queries that should not be exposed to end users or systems, and are useful for enforcing data quality constraints or transforming and enriching datasets that drive multiple declarative queries.
Here's a quick overview of how Databricks Delta Live Tables work:

Define Tables using SQL/Python: 
    Register Databricks Delta Live Tables using standard DDL SQL syntax or the Table API in Python. These tables define the data schema and associated views.
Express Transformations: 
    Directly define ETL logic for pipelines using ANSI SQL or Python on the tables.
Handle Lineage & Dependencies: 
    Databricks Delta Live Tables automatically track dependencies between tables and schedule execution based on table lineages.
Continuous/Scheduled Execution: 
    Pipelines can run continuously as data arrives or be scheduled to run at intervals, with new data incrementally processed.
Auto-Optimize Performance: 
    Databricks Delta Live Tables automatically optimize queries, utilize caching, manage indexes, partitions, and clusters to ensure optimal performance.
Unified Batch & Streaming: 
    Databricks Delta Live Tables process both streaming and batch data using the same table definitions and logic engine.
Handle Reliability & Scaling: 
    The service takes care of pipeline scaling, reliability, fault tolerance, and ensures exactly-once processing without requiring manual configuration.
Secure by Default: 
    You can implement fine-grained access controls on tables, with credential management, encryption, and other security aspects handled by the service. This ensures a secure default configuration.
...
Databricks Lakehouse Architecture to segment data quality - Databricks Delta Live Table
    Databricks Lakehouse Architecture to segment data quality into raw, refined, & aggregated tiers 
    (Source
    Databricks.com
    )
...
FAQs
    ...
    treaming tables, Materialized views, and Views.
